...
func (s *PrioritySemaphore) resize(n int) bool {
	...
	if status {
		s.log.Infof("[PK][HYPO-2] %s semaphore resized from %d to %d", s.name, cur, n)
...
func (s *PrioritySemaphore) release(key string) bool {
	...
	if _, ok := s.lockHolder[key]; ok {
		...
		s.log.Infof("[PK][HYPO-2] Lock %s has been released by %s. Available locks: %d", s.name, key, availableLocks)
		if s.pending.Len() > 0 {
			s.notifyWaiters()
		}
	}
...
func (s *PrioritySemaphore) tryAcquire(holderKey string) (bool, string) {
	...
	s.log.Infof("[PK][HYPO-2] %s", waitingMsg)
	...
	if s.acquire(holderKey) {
		s.pending.pop()
		s.log.Infof("[PK][HYPO-2] %s acquired by %s. Lock availability: %d/%d", s.name, holderKey, s.limit-len(s.lockHolder), s.limit)
		s.notifyWaiters()
		return true, ""
	}
	s.log.Infof("[PK][HYPO-2] Current semaphore %s holders. %v", s.name, s.lockHolder)
	return false, waitingMsg
}
