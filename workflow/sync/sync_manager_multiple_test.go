...
func testSyncManagersSemaphoreAcquisitionForDB(t *testing.T, dbType sqldb.DBType) {
	...
	// wf-01 releases lock
	syncMgr1.Release(wf01, wf01.Name, wf01.Spec.Synchronization)
	checkCannotAcquire(ctx, t, syncMgr2, wf04)
	checkCanAcquire(ctx, t, syncMgr1, wf03)
	// wf-03 releases lock
	syncMgr1.Release(wf03, wf03.Name, wf03.Spec.Synchronization)
...
func testSyncManagersSemaphoreAcquisitionWithPriorityForDB(t *testing.T, dbType sqldb.DBType) {
	...
	checkCannotAcquire(ctx, t, syncMgr2, wf02)
	// wf-04 releases lock
	syncMgr2.Release(wf04, wf04.Name, wf04.Spec.Synchronization)
	checkCannotAcquire(ctx, t, syncMgr2, wf02)
	checkCanAcquire(ctx, t, syncMgr1, wf03)
	// wf-03 releases lock
	syncMgr1.Release(wf03, wf03.Name, wf03.Spec.Synchronization)
		...
		for testCounter := range count {
			...
			for !acquired {
				...
				if acquired {
					...
					testMtx.Lock()
					sm.Release(wfCopy, wfCopy.Name, wfCopy.Spec.Synchronization)
					lockCount -= 1
					testMtx.Unlock()
				}
			}
