		...
		if wf.Status.Synchronization.Semaphore != nil {
			for _, holding := range wf.Status.Synchronization.Semaphore.Holding {
				...
				for _, holders := range holding.Holders {
					...
					log.Infof("[PK][HYPO-1] Acquiring key %s for workflow %s\n", resourceKey, wf.Name)
			...
			for _, holding := range wf.Status.Synchronization.Mutex.Holding {
				...
				if mutex == nil {
					...
					if holding.Holder != "" {
						...
						log.Infof("[PK][HYPO-1] got resource key %s for workflow %s\n", resourceKey, wf.Name)
...
func (cm *Manager) TryAcquire(wf *wfv1.Workflow, nodeName string, syncLockRef *wfv1.Synchronization) (bool, bool, string, error) {
	...
	if syncLockRef == nil {
		log.Infof("[PK][HYPO-1] syncLockRef was nil")
	...
	if err != nil {
		log.Infof("[PK][HYPO-1] unable to get syncLockName for workflow %s and nodeName %s\n", wf.Name, nodeName)
		return false, false, "", fmt.Errorf("requested configuration is invalid: %w", err)
	}
	log.Infof("[PK][HYPO-1] syncLockName: %+v for workflow %s and nodeName %s\n", syncLockName, wf.Name, nodeName)

	lockKey := syncLockName.EncodeName()
	log.Infof("[PK][HYPO-1] lockKey: %s for workflow %s and nodeName %s\n", lockKey, wf.Name, nodeName)
	...
	log.Infof("[PK][HYPO-1] holderKey: %s for workflow %s and nodeName %s\n", holderKey, wf.Name, nodeName)
	var priority int32
...
func (cm *Manager) Release(wf *wfv1.Workflow, nodeName string, syncRef *wfv1.Synchronization) {
	if syncRef == nil {
		log.Infof("[PK][HYPO-1]Release called but syncRef was nil")
		return
	}
	...
	holderKey := getHolderKey(wf, nodeName)
	log.Infof("[PK][HYPO-1] Got holder key %s for workflow %s and node %s\n", holderKey, wf.Name, nodeName)
	lockName, err := GetLockName(syncRef, wf.Namespace)

	if err != nil {
		log.Infof("[PK][HYPO-1] Error while getting lockname for workflow %s and node %s", wf.Name, nodeName)
		return
	}
	log.Infof("[PK][HYPO-1] Got lock name %+v for workflow %s and node %s", lockName, wf.Name, nodeName)
...
func (cm *Manager) ReleaseAll(wf *wfv1.Workflow) bool {
	...
	if wf.Status.Synchronization.Semaphore != nil {
		...
		for _, waiting := range wf.Status.Synchronization.Semaphore.Waiting {
			...
			log.Infof("[PK][HYPO-1] removing resource key %s from queue for workflow %s\n", resourceKey, wf.Name)
	...
	if wf.Status.Synchronization.Mutex != nil {
		for _, holding := range wf.Status.Synchronization.Mutex.Holding {
			...
			resourceKey := getResourceKey(wf.Namespace, wf.Name, holding.Holder)
			log.Infof("[PK][HYPO-1] releasing resource key %s from queue for workflow %s\n", resourceKey, wf.Name)
			syncLockHolder.release(resourceKey)
