...
func TestCronWorkflowConditionSubmissionError(t *testing.T) {
	...
	testMetrics, err := metrics.New(context.Background(), telemetry.TestScopeName, telemetry.TestScopeName, &telemetry.Config{})
	require.NoError(t, err)
	woc := &cronWfOperationCtx{
		wfClientset:       cs,
...
func TestSpecError(t *testing.T) {
	...
	cs := fake.NewSimpleClientset()
	ctx := context.Background()
	testMetrics, err := metrics.New(ctx, telemetry.TestScopeName, telemetry.TestScopeName, &telemetry.Config{})
	require.NoError(t, err)
	woc := &cronWfOperationCtx{
		wfClientset: cs,
		wfClient:    cs.ArgoprojV1alpha1().Workflows(""),
...
func TestScheduleTimeParam(t *testing.T) {
	...
	cs := fake.NewSimpleClientset()
	testMetrics, _ := metrics.New(context.Background(), telemetry.TestScopeName, telemetry.TestScopeName, &telemetry.Config{})
	woc := &cronWfOperationCtx{
		wfClientset:       cs,
		wfClient:          cs.ArgoprojV1alpha1().Workflows(""),
		cronWfIf:          cs.ArgoprojV1alpha1().CronWorkflows(""),
