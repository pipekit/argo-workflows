...
func (woc *wfOperationCtx) processPodSpecPatch(ctx context.Context, tmpl *wfv1.Template, pod *apiv1.Pod) ([]string, error) {
	...
	for _, patch := range toProcess {
		...
		processedTmpl, err := common.ProcessArgs(ctx, newTmpl, &wfv1.Arguments{}, woc.globalParams, localParams, false, false, woc.wf.Namespace, woc.controller.configMapInformer.GetIndexer())
		if err != nil {
