...
func (d *dagContext) getTaskNode(taskName string) *wfv1.NodeStatus {
	...
	if err != nil {
		d.log.Warnf("was unable to obtain the node for %s, taskName %s", nodeID, taskName)
		return nil
	}
	return node
}
...
func (d *dagContext) assessDAGPhase(targetTasks []string, nodes wfv1.Nodes, isShutdown bool) (wfv1.NodePhase, error) {
	...
	d.log.Warnf("[PK] assesing DAG phase for %v, all tasks %+v", targetTasks, d.tasks)
	if isShutdown {
		d.log.Warnf("[PK] shutdown")
	...
	if err != nil {
		d.log.Warnf("[PK] ADP failed boundary node")
		return "", err
	}
	d.log.Warnf("[PK] Got boundaryNode with phase %s name %s and id %s", boundaryNode.Phase, boundaryNode.Name, boundaryNode.ID)
	...
	for !uniqueQueue.empty() {
		...
		if err != nil {
			...
			d.log.Warnf("[PK] ADP still running")
			return wfv1.NodeRunning, nil
		}
		d.log.Warnf("[PK] Phase %s for ancestor %s. Current is %+v\n", node.Phase, node.Name, curr)
		...
		if previousPhase, isTargetTask := targetTaskPhases[node.ID]; isTargetTask {
			...
			if !previousPhase.FailedOrError() {
				d.log.Warnf("[PK] updated node %s to phase %s", node.Name, branchPhase)
		...
		if node.Type == wfv1.NodeTypeRetry {
			...
			d.log.Warnf("[PK] generating new children with phase %s", branchPhase)
	...
	for _, depName := range targetTasks {
		...
		if branchPhase == "" {
			...
			if failFast {
				d.log.Warnf("[PK] ADP failfast")
				break
			}
		}
	}

	d.log.Warnf("[PK] ADP result %v", result)
	return result, nil
}

type NanoSecondHook struct {
}

var (
	controllerStartTime int64
	globalExecDAGCount  atomic.Uint64
)

func init() {
	// init is guaranteed to be single threaded
	t := time.Now().UnixNano()
	controllerStartTime = t
	globalExecDAGCount.Store(0)
}

func (hook *NanoSecondHook) Fire(entry *logrus.Entry) error {

	// given the controllerStartTime is constant after init, this should be safe
	elapsedTime := time.Now().UnixNano() - controllerStartTime
	entry.Data["nanoseconds"] = strconv.FormatInt(elapsedTime, 10)
	return nil
}

func (hook *NanoSecondHook) Levels() []logrus.Level {
...
func (woc *wfOperationCtx) executeDAG(ctx context.Context, nodeName string, tmplCtx *templateresolution.Context, templateScope string, tmpl *wfv1.Template, orgTmpl wfv1.TemplateReferenceHolder, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	...
	log.Warnf("[PK] dagPhase is %s", dagPhase)

	switch dagPhase {
	case wfv1.NodeRunning:
		log.Warnf("[PK] dagPhase returns early")
		return node, nil
	case wfv1.NodeError, wfv1.NodeFailed:
		...
		log.Warnf("[PK] dagPhase updated for %s to %s", nodeName, dagPhase)
		log.Warnf("[PK] dagPhase returns early after outbound")
		return node, nil
	}
	...
	if err != nil {
		log.Warnf("[PK][KNOWNBAD] Failing because of template outputs")
	...
	if outputs != nil {
		node, err = woc.wf.GetNodeByName(nodeName)
		if err != nil {
			woc.log.Errorf("[KNOWNBAD]unable to get node by name for %s", nodeName)
			return nil, err
		}
	...
	if node.MemoizationStatus != nil {
		...
		if err != nil {
			woc.log.WithFields(logrus.Fields{"nodeID": node.ID}).WithError(err).Error("Failed to save node outputs to cache")
	...
	err = woc.updateOutboundNodesForTargetTasks(dagCtx, targetTasks, nodeName)
	if err != nil {
		woc.log.Errorf("[KNOWNBAD] unable to update outbound nodes %s", err)
		return nil, err
	}
	log.Warnf("[PK] dagPhase updated for %s to %s", nodeName, dagPhase)
...
func (woc *wfOperationCtx) updateOutboundNodesForTargetTasks(dagCtx *dagContext, targetTasks []string, nodeName string) error {
	woc.log.Warnf("[PK] updateOutboundNodesForTargetTasks %s >%v<", nodeName, targetTasks)
	...
	woc.wf.Status.Nodes.Set(node.ID, *node)
	woc.log.Warnf("[PK] Outbound nodes of %s set to %s", node.ID, outbound)
	return nil
}
...
func (woc *wfOperationCtx) executeDAGTask(ctx context.Context, dagCtx *dagContext, taskName string) {
	...
	connectDependencies := func(taskNodeName string) {
		if len(taskDependencies) == 0 || taskGroupNode != nil {
			...
			log.Warnf("[PK] Connecting boundary as parent of %s", taskNodeName)
			...
			for _, depName := range taskDependencies {
				log.Warnf("[PK] Connecting task %s as parent of %s", depName, taskNodeName)
				depNode := dagCtx.getTaskNode(depName)
				outboundNodeIDs := woc.getOutboundNodes(depNode.ID)
				log.Warnf("[PK] Outbound nodes ids for %s are >%v<", depName, outboundNodeIDs)
	...
	if dagCtx.GetTaskDependsLogic(taskName) != "" {
		...
		if !execute {
			...
			log.Warnf("[PK] Marking %s as omitted", nodeName)
	...
	for _, taskName := range d.GetTaskDependencies(taskName) {
		...
		if depNode.Type == wfv1.NodeTypeTaskGroup {
			...
			for _, childNodeID := range depNode.Children {
				...
				if err != nil {
					d.log.Warnf("was unable to obtain node for %s", childNodeID)
