...
func TestPhaseBasedRebuilder(t *testing.T) {
	t.Run("does not rebuild when succeeded", func(t *testing.T) {
		getState := func(key Key) TaskState {
			return TaskStateSucceeded
		}
		rebuilder := NewPhaseBasedRebuilder[Key, testValueWithHash](getState, nil)
		ctx := context.Background()
	...
	t.Run("uses depends logic when provided", func(t *testing.T) {
		...
		evaluateDepends := func(ctx context.Context, key Key, fetch Fetch[Key, testValueWithHash]) (bool, bool, error) {
	...
	t.Run("handles nil getState function", func(t *testing.T) {
		rebuilder := NewPhaseBasedRebuilder[Key, testValueWithHash](nil, nil)
		ctx := context.Background()
...
func TestStateBasedRebuilder(t *testing.T) {
	t.Run("rebuilds only when pending", func(t *testing.T) {
		states := map[Key]TaskState{
			...
			"skipped":   TaskStateSkipped,
			"omitted":   TaskStateOmitted,
			"error":     TaskStateError,
		}
		getState := func(key Key) TaskState {
			return states[key]
		}
		rebuilder := NewStateBasedRebuilder[Key, testValueWithHash](getState)
		ctx := context.Background()
	...
	t.Run("handles nil getState function", func(t *testing.T) {
		rebuilder := NewStateBasedRebuilder[Key, testValueWithHash](nil)
		ctx := context.Background()
