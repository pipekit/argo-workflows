...
func TestArtifactLocation_MultiplePluginTypes(t *testing.T) {
	...
	t.Run("PluginKeyOperations", func(t *testing.T) {
		location := &ArtifactLocation{
			Plugin: &PluginArtifact{
				Name:          "test-plugin",
				Configuration: `{"bucket": "my-bucket"}`,
		...
		key, err := location.GetKey()
		assert.NoError(t, err)
		assert.Equal(t, "original/path", key)
		...
		newKey, err := location.GetKey()
		assert.NoError(t, err)
		assert.Equal(t, "new/path/to/artifact", newKey)
		assert.Equal(t, "new/path/to/artifact", location.Plugin.Key)

		// Test AppendToKey
		err = location.AppendToKey("subdir/file.txt")
		assert.NoError(t, err)

		finalKey, err := location.GetKey()
		assert.NoError(t, err)
		assert.Equal(t, "new/path/to/artifact/subdir/file.txt", finalKey)
	})
}
